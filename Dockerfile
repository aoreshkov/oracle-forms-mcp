...
# quote it as one argument: --convert-command "/opt/forms/convert.sh --xml {}". A converter of its own
# for .pll libraries goes in --compile-command / OFMCP_COMPILE_COMMAND the same way. Both mounts must
